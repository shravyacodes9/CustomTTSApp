...
class TTSApp:
    ...
    def _build_gui(self):
        ...
        # Main body: Center all content
        container = tk.Frame(self.root, bg="#fae2b5")
        container.grid(row=1, column=1, sticky="nsew", padx=20, pady=16)
        self.root.grid_rowconfigure(1, weight=1)
        self.root.grid_columnconfigure(0, weight=1)
        self.root.grid_columnconfigure(1, weight=0)
        self.root.grid_columnconfigure(2, weight=1)
        
        # Create inner container for better centering
        inner_container = tk.Frame(container, bg="#fae2b5")
        inner_container.grid(row=0, column=0, sticky="nsew")
        inner_container.grid_columnconfigure(0, weight=1)
        inner_container.grid_columnconfigure(1, weight=0)
        inner_container.grid_columnconfigure(2, weight=0)
        inner_container.grid_rowconfigure(0, weight=1)

        # LEFT: Input & Output 
        text_area = tk.Frame(inner_container, bg="#fff7ef", bd=1, relief="flat")
        text_area.grid(row=0, column=0, padx=(0, 12), pady=0, sticky="nsew")
        ...
        sidebar_container = tk.Frame(inner_container, bg="#fae2b5")
        sidebar_container.grid(row=0, column=1, columnspan=2, padx=(12, 0), pady=0, sticky="nsew")
        ...
        # Customization Sidebar (larger and wider)
        custom_frame = tk.Frame(sidebar_container, bg="#f7ddb3", bd=2, relief="ridge", height=680, width=320)
        ...
        tk.Label(custom_frame, text="Customization", bg="#f7ddb3", font=("Arial", 16, "bold"), fg="#7d482e").grid(row=0, column=0, columnspan=2, pady=(12,5))
        tk.Label(custom_frame, text="Font:", bg="#f7ddb3", font=("Arial", 11)).grid(row=1, column=0, sticky="w", padx=15, pady=6)
        ...
        font_menu.config(width=18, font=("Arial", 11))
        font_menu.grid(row=1, column=1, sticky="ew", padx=(0,15), pady=6)
        tk.Label(custom_frame, text="Font Size:", bg="#f7ddb3", font=("Arial", 11)).grid(row=2, column=0, sticky="w", padx=15, pady=6)
        ...
        tk.Scale(custom_frame, variable=self.size_var, from_=12, to=32, orient="horizontal", showvalue=True, bg="#f7ddb3",
                 command=lambda evt: self._refresh_custom_style(), length=160).grid(row=2, column=1, sticky="ew", padx=(0,15), pady=6)
        tk.Label(custom_frame, text="Word Spacing:", bg="#f7ddb3", font=("Arial", 11)).grid(row=3, column=0, sticky="w", padx=15, pady=6)
        ...
        tk.Scale(custom_frame, variable=self.ws_var, from_=0, to=8, orient="horizontal", showvalue=True, bg="#f7ddb3",
                 command=lambda evt: self._refresh_custom_style(), length=160).grid(row=3, column=1, sticky="ew", padx=(0,15), pady=6)
        tk.Label(custom_frame, text="Letter Spacing:", bg="#f7ddb3", font=("Arial", 11)).grid(row=4, column=0, sticky="w", padx=15, pady=6)
        ...
        tk.Scale(custom_frame, variable=self.ls_var, from_=0, to=4, orient="horizontal", showvalue=True, bg="#f7ddb3",
                 command=lambda evt: self._refresh_custom_style(), length=160).grid(row=4, column=1, sticky="ew", padx=(0,15), pady=6)
        tk.Button(custom_frame, text="Background", command=self._choose_bg_color, bg="#ffefcb", width=18, font=("Arial", 10, "bold")).grid(row=5, column=0, pady=(14,4), padx=15)
        tk.Button(custom_frame, text="Text Color", command=self._choose_fg_color, bg="#ffefcb", width=18, font=("Arial", 10, "bold")).grid(row=5, column=1, pady=(14,4), padx=15)
        tk.Button(custom_frame, text="Highlight Color", command=self._choose_hl_color, bg="#ffefcb", width=18, font=("Arial", 10, "bold")).grid(row=6, column=0, columnspan=2, pady=(4,8), padx=15, sticky="ew")
        tk.Label(custom_frame, text="Highlight Mode:", bg="#f7ddb3", font=("Arial", 11)).grid(row=7, column=0, sticky="w", padx=15, pady=(12,6), columnspan=2)
        self.highlight_var = tk.StringVar(value=self.settings.get("highlight_mode", "word"))
        tk.Radiobutton(custom_frame, text="Word", variable=self.highlight_var, value="word", bg="#f7ddb3", font=("Arial", 10), command=self._refresh_highlight_mode).grid(row=8, column=0, sticky="w", padx=(25,0), pady=4)
        tk.Radiobutton(custom_frame, text="Line", variable=self.highlight_var, value="line", bg="#f7ddb3", font=("Arial", 10), command=self._refresh_highlight_mode).grid(row=8, column=1, sticky="w", padx=(15,0), pady=4)
        tk.Button(custom_frame, text="Save Prefs", command=self.save_settings,
                  bg="#f7ca97", font=("Arial", 11, "bold"), width=18).grid(row=9, column=0, pady=(16,6), padx=15)
        tk.Button(custom_frame, text="About", command=self.show_about_popup,
                  bg="#f7ca97", font=("Arial", 11, "bold"), width=18).grid(row=9, column=1, pady=(16,6), padx=15)

        # TTS Controls Sidebar (larger and wider)
        tts_frame = tk.Frame(sidebar_container, bg="#f5dab0", bd=2, relief="ridge", height=680, width=300)
        ...
        tk.Label(tts_frame, text="TTS Controls", bg="#f5dab0", font=("Arial", 16, "bold"), fg="#995c00")\
            .grid(row=0, column=0, columnspan=2, pady=(12,10))
        tk.Button(tts_frame, text="Play", command=self.play_tts, width=14, bg="#dec09c", font=("Arial", 12, "bold"), height=2).grid(row=1, column=0, padx=12, pady=(8,6))
        tk.Button(tts_frame, text="Pause", command=self.pause_tts, width=14, bg="#dec09c", font=("Arial", 12, "bold"), height=2).grid(row=1, column=1, padx=12, pady=(8,6))
        tk.Button(tts_frame, text="Resume", command=self.resume_tts, width=14, bg="#dec09c", font=("Arial", 12, "bold"), height=2).grid(row=2, column=0, padx=12, pady=6)
        tk.Button(tts_frame, text="Stop", command=self.stop_tts, width=14, bg="#dec09c", font=("Arial", 12, "bold"), height=2).grid(row=2, column=1, padx=12, pady=6)
        tk.Label(tts_frame, text="Reading Speed:", bg="#f5dab0", font=("Arial", 12)).grid(row=3, column=0, columnspan=2, sticky="w", padx=18, pady=(20,6))
        self.speed_var = tk.IntVar(value=170)
        tk.Scale(tts_frame, variable=self.speed_var, from_=120, to=250, orient="horizontal",
                 showvalue=True, bg="#f5dab0", command=lambda e:self._refresh_speed(), length=200).grid(row=4, column=0, columnspan=2, padx=18, pady=(0,12))
